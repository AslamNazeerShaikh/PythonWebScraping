...
## Corpus Check
- 44 files · ~31,957 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 510 nodes · 867 edges · 30 communities (22 shown, 6 thin omitted)
- Extraction: 97% EXTRACTED · 3% INFERRED · 0% AMBIGUOUS · INFERRED: 24 edges (avg confidence: 0.92)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `10556ddc`
...
- `test_get_returns_default_when_unset()` --calls--> `_get()`  [EXTRACTED]
...
Cohesion: 0.05
Nodes (60): fixture, Page, build_search_url(), check_bot_wall(), enrich_job(), handle_possible_block(), human_scroll(), is_relevant() (+52 more)

### Community 1 - "main.py"
Cohesion: 0.06
Nodes (57): command, DeclarativeBase, Session, Central configuration — every tunable lives here, loaded from `.env`.…, All runtime settings. Grouped by concern; see `.env.example`., Settings, count_jobs(), fetch_all_jobs() (+49 more)

### Community 2 - "FakeContext"
Cohesion: 0.09
Nodes (27): Exception, _extension_args(), Build ``--load-extension`` args for uBOL (headed mode only). Headless-shell…, fake_playwright(), FakeBrowserType, FakeContext, FakeRoute, Context fake: serves a preset page, records route handlers. (+19 more)
...
Cohesion: 0.05
Nodes (37): 10. Testing architecture (why 100% is feasible), 11. Knowledge graph + living docs, 12. Fair use (binding), 1. Project structure (where everything lives), 2. Start point (how the app boots), 3. Endpoints (CLI commands — this app has no HTTP server), 4. Code style (the conventions used everywhere), 5.1 Layered architecture (the backbone) (+29 more)

### Community 13 - "Job"
Cohesion: 0.09
Nodes (29): field_validator, Job, JobRow, BaseModel, Layer 3 (validation: Pydantic) + Layer 4 (persistence: SQLAlchemy ORM). Data…, Build a new ORM row from a validated :class:`Job`., Convert a DB row back into a validated :class:`Job`., One validated job record. Attributes: job_id: Stable Naukri numeric id parsed… (+21 more)
...
## Knowledge Gaps
- **104 isolated node(s):** `$schema`, `plugin`, `Usage`, `What graphify is for`, `Step 0 - GitHub repos and multi-path merge (only if a URL or several paths)` (+99 more)
...
- **Why does `Job` connect `Job` to `test_scraper.py`, `main.py`?**
